...
def write_lammps_data(filename, atoms, atom_types, comment=None, cutoff=None,
    ...
    if bond_types:
        ...
        # This is pymatnest implementation...

        #from matscipy.neighbours import neighbour_list
        #i_list, j_list = neighbour_list('ij', atoms, cutoff)

        # ...this is ASE implementation
        nl = NeighborList(len(atoms)*[cutoff/2.0],self_interaction=False,bothways=True)
        nl.update(atoms)
        i_list = list()
        j_list = list()
        for i, atom in enumerate(atoms):
            indices, offsets = nl.get_neighbors(i)
            i_list.extend([ i for j in indices ])
            j_list.extend([ j for j in indices ])
